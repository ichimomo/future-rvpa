...
#    SRdata <- get.SRdata(res.pma,years=1990:2000) # 特定の期間のデータだけを使う場合
    ...
    # 計算時間を短縮するいくつかのオプション
    # 通常計算(N=500回で78秒)
    system.time(a1 <- est.MSY(res.pma,fres.HS4$input,nyear=50,N=500,PGY=NULL,B0percent=NULL))
    # グリッドサーチ（かえって遅くなる、92秒）
    system.time(a2 <- est.MSY(res.pma,fres.HS4$input,nyear=50,N=500,PGY=NULL,B0percent=NULL,optim.method="grid"))
    ...
    system.time(a3 <- est.MSY(res.pma,fres.HS4$input,nyear=50,N=500,PGY=NULL,B0percent=NULL,calc.yieldcurve=FALSE))        
...
future.vpa <-
  function(res0,
           ...
           HCR=NULL,# HCRを使う場合、list(Blimit=154500, Bban=49400)のように指定,
    ...
    if (waa.fun){ #年齢別体重の予測関数
      ...
      NAA <- res0$naa
      al <- nrow(WAA)
      WAA.res <- lapply(1:al, function(i) {
        log.w <- as.numeric(log(WAA[i,]))
        log.n <- as.numeric(log(NAA[i,]))
        lm(log.w~log.n)
      })
      WAA.cv <- sapply(1:al, function(i) sqrt(mean(WAA.res[[i]]$residuals^2)))
      WAA.b0 <- sapply(1:al, function(i) as.numeric(WAA.res[[i]]$coef[1]))
      WAA.b1 <- sapply(1:al, function(i) as.numeric(WAA.res[[i]]$coef[2]))
      waa.rand <- array(0,dim=c(al,nyear+1-min.age,N+1))
      set.seed(0)
      for (ii in 1:(N+1)) {
        if (ii==1) {
          waa.rand[,,ii] <- t(sapply(1:al, function (j) rnorm(nyear+1-min.age,0,0)))
        } else {
          waa.rand[,,ii] <- t(sapply(1:al, function (j) rnorm(nyear+1-min.age,-0.5*WAA.cv[j]^2,WAA.cv[j])))
        }
      }
    }
    ...
    for(k in 1:N){
        ...
        if(start.year==(max(years)+1)){ 
          ...
          if(is.na(naa[1,1,k])){
            if(fyears[1]-min.age < start.year){
              ...
              {if(res0$ssb.coef >= 0)  {
                if (waa.fun) {
                  waa[2:al,1,k] <- sapply(2:al, function(ii) as.numeric(exp(WAA.b0[ii]+WAA.b1[ii]*log(naa[ii,1,k])+waa.rand[ii,1,k])))
                }
                thisyear.ssb <- sum(naa[,1,k]*waa[,1,k]*maa[,1,k]*exp(-res0$ssb.coef*(M[,1,k]+faa[,1,k])),na.rm=T)*res0$input$unit.waa/res0$input$unit.biom
            ...
            if (waa.fun) {
              waa[1,1,k] <- as.numeric(exp(WAA.b0[1]+WAA.b1[1]*log(naa[1,1,k])+waa.rand[1,1,k])) 
            }
      ...
      if(!is.null(rec.new)){
        if(!is.list(rec.new)){
          ...
          naa[1,fyears%in%rec.new$year,k] <- rec.new$rec
        }}
      ...
      for(i in 1:(ntime-1)){
        ...
        if(fyears[i+1]-min.age < start.year){
          ...
          if(ts==1){
            if(is.null(res0$ssb.coef)) res0$ssb.coef <- 0
            if (waa.fun) {
              waa[2:al,i+1-min.age,k] <- sapply(2:al, function(ii) as.numeric(exp(WAA.b0[ii]+WAA.b1[ii]*log(naa[ii,i+1-min.age,k])+waa.rand[ii,i+1-min.age,k])))
            }
        ...
        if(is.na(naa[1,i+1,k]) && (floor(fyears[i+1])-fyears[i+1])==0){ # 加入は最初の季節にのみおこる
          ...
          if (waa.fun) {
            waa[1,i+1,k] <- as.numeric(exp(WAA.b0[1]+WAA.b1[1]*log(naa[1,i+1,k])+waa.rand[1,i+1,k])) 
          }
        }
        else{
        ...
        ## HCRを使う場合(来年のFを変更する)
        if(!is.null(HCR)&&i<nyear){
            ssb.tmp <- sum(naa[,i,k]*waa[,i,k]*maa[,i,k]*exp(-res0$ssb.coef*(M[,i,k]+faa[,i,k])),na.rm=T)*
                                               res0$input$unit.waa/res0$input$unit.biom                        
            if(ssb.tmp>HCR$Blimit) faa[,i+1,k] <- faa[,i+1,k]
            else faa[,i+1,k] <- (ssb.tmp-HCR$Bban)/(HCR$Blimit-HCR$Bban)*faa[,i+1,k]
            faa[,i+1,k] <- ifelse(faa[,i+1,k]<0,0,faa[,i+1,k])
        }          
      }
...
##
get.stat <- function(fout,eyear=NULL,hsp=NULL,tmp.year=NULL){
...
get.stat2 <- function(fout,unit.waa=1,eyear=NULL,hsp=NULL,tmp.year=NULL){
    col.target <- ifelse(fout$input$N==0,1,-1)     
    if(is.null(tmp.year)) tmp.year <- (nrow(fout$vwcaa)-eyear):nrow(fout$vwcaa)
        nage <- dim(fout$naa)[[1]]
...
#            ssb.array[,,i] <- tmp$vssb
    ...
    farg.tmp$N <- N
    trace.N <- ifelse(isTRUE(calc.yieldcurve),N,0)    
...
get.perform <- function(fout0,Blimit=0,longyear=50,smallcatch=0.5,N=NULL,
                        shortyear=c(3,5,10),tmp.year=tmp.year){
...
fit.SR <- function(SRdata,SR="HS",method="L2",AR=1,gamma=0.001,TMB=FALSE,hessian=FALSE,w=rep(1,length(SRdata$year))){
  ...
  obj.f <- function(a,b,sigma,rho){
    ...
    sigma2 <- c(sigma/sqrt(1-rho^2), rep(sigma,N-1))
    
    if (method == "L2") {
      obj <- -sum(w*dnorm(resid2,0,sigma2,log=TRUE))
    } else {
      obj <- -sum(w*sapply(1:N, function(i){-log(2*sigma2[i])-abs(resid2[i]/sigma2[i])}))
    }
    return(obj)
  }
  ...
  if (SR == "HS") { 
    ...
    if (AR == 0) {
      ...
      obj.f2 <-  function(x) obj.f(exp(x[1]),exp(x[2]),exp(x[3]),1/(1+exp(-x[4])))
      init <- c(init,0)
    }
  }
  ...
  if (!isTRUE(TMB)) {
    if (AR == 0) {
      grids <- expand.grid(seq(opt$par[1]-2,opt$par[1]+2,len=5),seq(opt$par[2]-2,opt$par[2]+2,len=5),opt$par[3])
    } else {
      grids <- expand.grid(seq(opt$par[1]-2,opt$par[1]+2,len=5),seq(opt$par[2]-2,opt$par[2]+2,len=5),opt$par[3],opt$par[4])
    }
    init <- as.numeric(grids[which.min(apply(grids,1,obj.f2)),])
    opt <- optim(init,obj.f2)
  }
  ...
  sigma <- exp(opt$par[3])
  if (method!="L2") sigma <- sqrt(2)*sigma
  ...
  if (method=="L2") {
    if (TMB) {
      ...
      params$log_sigma <- log(Res$pars[3]/(sqrt(1-Res$pars[4]^2)))
...
boot.SR <- function(Res,n=100,seed=1){
  ...
  sigma <- sapply(1:N, function(i) ifelse(i==1,Res$pars$sigma/sqrt(1-Res$pars$rho^2),Res$pars$sigma))
  
  set.seed(seed)
  lapply(1:n, function(j){
    N <- length(Res$input$SRdata$SSB)
    resids <- rnorm(N,0,sigma)
...
### profile likelihood
prof.lik <- function(Res,a=Res$pars$a,b=Res$pars$b,sigma=Res$pars$sigma,rho=Res$pars$rho) {
