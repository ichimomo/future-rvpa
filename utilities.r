...
Plot_HCR <- function(SBtarget,SBlim,SBban,Ftarget,beta=0.8,col.multi2currf="blue",col.SBtarget="green",col.SBlim="yellow",col.SBban="red",col.Ftarget="black",col.betaFtarget="gray"){
  ...
  ggplct <- ggplot(data.frame(x = c(0,1.5*SBtarget),y= c(0,1.5*Ftarget)), aes(x=x)) + stat_function(fun = h,lwd=2,color=col.multi2currf)
  ggplct  + geom_vline(xintercept = SBtarget, size = 0.9, linetype = 2, color = col.SBtarget) +
  geom_vline(xintercept = SBlim, size = 0.9, linetype = 2, color = col.SBlim) +
  geom_vline(xintercept = SBban, size = 0.9, linetype = 2, color = col.SBban) +
  geom_hline(yintercept = Ftarget, size = 0.9, linetype = 2, color = col.Ftarget) +
  geom_hline(yintercept = beta*Ftarget, size = 0.7, linetype = 2, color = col.betaFtarget) +
  labs(title = "",subtitle = "", caption =  "", x = "SSB (×1000 [t])", y = "multiplier to current F",color = "") +
  annotate("text", label="SBtarget", x=SBtarget, y=1.2*Ftarget, fontface="bold") +
  annotate("text", label="SBlimit", x=SBlim, y=1.1*Ftarget, fontface="bold") +
  annotate("text", label="SBban", x=SBban, y=1.2*Ftarget, fontface="bold") +
  annotate("text", label="Ftarget", x=SBtarget/15, y=0.95*Ftarget) +
  annotate("text", label="β Ftarget", x=SBtarget/15, y=0.95*beta*Ftarget) +
  theme_classic()
